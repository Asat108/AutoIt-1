...
;_Debugout($oRS.Fields("zip").value)
;_Debugout($oRS.Fields("zone").value)
...
For $iIndex = 1 To $oRS.RecordCount
	...
	$zipleft3 = StringTrimRight($zip, 3)
	
	_DebugOut( "Zip left 3"&$zipleft3 )
	_DebugOut( $zipleft3 )
...
;For $iIndex = 1 To $oRS.RecordCount
$iIndex = 1
While $iIndex <= GUICtrlRead($catalogs_to_print)
