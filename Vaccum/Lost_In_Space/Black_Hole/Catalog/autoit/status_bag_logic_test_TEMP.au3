...
$mainwindow = GUICreate("Order Number", 200, 130)
...
_Debugout ("BPM Match "&$BPM_Batch)
$sql = "Select * FROM CF where CF.Batch like '"&$BPM_Batch&"'"

$oRS.Open($sql, $oConn, 1, 3)

For $iIndex = 1 To $oRS.RecordCount
;   _debugout($iIndex)
	$order_number = $oRS.Fields("order").value
	$zip = $oRS.Fields("Zip").value
;    $zone = $oRS.Fields
    $zip = StringTrimLeft($zip, 2)
...
_debugout ("Debugging, $zone variable"&$zoneBPM)
_debugout ("Debugging SQL Statment")
	$sql_update2 = "UPDATE CF SET Zone_Old='"&$zoneBPM&"' WHERE 'CF.ORDER='"&$order_number&"' AND CF.Batch like '"&$BPM_Batch&"'"
	_debugout($zoneBPM)
	_debugout("BPM Batch"&$BPM_Batch)
