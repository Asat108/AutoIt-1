...
;	$array = StringSplit($clean, ",")
	...
	If StringInStr($order[13], "Visa") and StringInStr($order[14], "6") Then  ; array[13] is cardtype if present
		$cctype = $visaNum
		ccheckout($order)
	EndIf
	If StringInStr($order[13], "American Express") and StringInStr($order[14], "6") Then
		$cctype = $amexNum
		ccheckout($order)
	EndIf
	If StringInStr($order[13], "Discover") and StringInStr($order[14], "6") Then
		$cctype = $discNum
		ccheckout($order)
	EndIf
	If StringInStr($order[13], "Mastercard") and StringInStr($order[14], "6") Then
		$cctype = $masterNum
		ccheckout($order)
	EndIf
